...
**Date:** 2026-04-08  
**Status:** Active  
**Team:** pm_bot, dev_bot, qa_bot, git_bot
...
|-------|------|-------|------------------|
| **pm_bot** | Project Manager | xiaomi/mimo-v2-flash | Orchestration, planning, reporting |
| **dev_bot** | Lead Developer | kwaipilot/kat-coder-pro-v2 | Go implementation, code generation |
| **qa_bot** | Quality Gatekeeper | kwaipilot/kat-coder-pro-v2 | Go code review, security, testing |
| **git_bot** | GitHub Operations | kwaipilot/kat-coder-pro-v2 | Commits, PRs, CI/CD monitoring |
| **py_bot** | Python Developer | kwaipilot/kat-coder-pro-v2 | Python/FastAPI, Docker, SSH automation |
...
└───────────────────────────┬───────────────────────────────────────┘
            ...
            ▼               ▼               ▼               ▼
┌───────────────────┐ ┌───────────────────┐ ┌───────────────────┐ ┌───────────────────┐
│  dev_bot          │ │  py_bot           │ │  qa_bot           │ │  git_bot          │
│  • Implements     │ │  • Implements     │ │  • Reviews code   │ │  • Commits & PRs │
│    Go             │ │    Python/FastAPI │ │  • Finds bugs     │ │  • Repo mgmt     │
│  • Writes tests   │ │  • Writes tests   │ │  • Security audit │ │  • CI/CD monitor │
│  • kwaipilot/kat │ │  • kwaipilot/kat  │ │  • kwaipilot/kat  │ │  • kwaipilot/kat │
└───────────────────┘ └───────────────────┘ └───────────────────┘ └───────────────────┘
            │               │               │
            │               │               │
            ▼               ▼               ▼
...
| **WORKLOG.md** | Append-only action log (MANDATORY) | Markdown files |
...
7. ✅ **WORKLOG.md appended** with all significant actions
8. ✅ PM confirmed delivery
9. ✅ Human acceptance received
...
7. **pm_bot**: **Creates WORKLOG.md** in project root (append-only action log)
8. **pm_bot**: Assigns first task to dev_bot via `TASK.md`

### 6.2 Task Execution

1. **dev_bot**: Reads `TASK.md`
2. **dev_bot**: **Appends WORKLOG.md** — task started
3. **dev_bot**: Implements feature
4. **dev_bot**: Writes tests
5. **dev_bot**: Runs `go fmt ./... && go vet ./...`
6. **dev_bot**: Runs tests `go test ./... -v`
7. **dev_bot**: **Appends WORKLOG.md** — implementation complete
8. **dev_bot**: Creates `DEV_HANDOVER.md`
9. **dev_bot** → pm_bot: "Ready for review"

### 6.3 Code Review

1. **pm_bot** → qa_bot: Review request with `DEV_HANDOVER.md`
2. **qa_bot**: **Appends WORKLOG.md** — review started
3. **qa_bot**: Reviews code
4. **qa_bot**: Creates `QA_REPORT.md`
5. **qa_bot**: **Appends WORKLOG.md** — review complete
6. **qa_bot** → pm_bot: "Review complete [status]"
